...
const CampaignCard = props => {
  const {
    data: { name, socialLinks, posts, weeklyPosts, influencers, daysLeft, status, by },
  } = props;
  ...
  return (
    <div className={styles.campaignCard}>
      ...
      <div className={styles.weekCount}>
        <h5>{weeklyPosts ? weeklyPosts : '-'}</h5>
      </div>
      <div className={styles.influencers}>
        <h5>{influencers ? influencers : '-'}</h5>
      </div>
      <div className={styles.daysLeft}>
        <h5>{daysLeft?.count ? daysLeft.count : '-'}</h5>

        {daysLeft?.end && <span>Ends {daysLeft.end}</span>}
      </div>
